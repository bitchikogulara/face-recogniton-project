...
A local-first smart home control system that uses on-device face recognition to authenticate users and send commands to smart devices — door locks, lights, cameras, and more.
...
**Mobile app** — captures the user's face, runs liveness detection and recognition on-device, mints a signed auth token on success, and sends the command to the gateway over an encrypted connection.
...
**Edge devices** — ESP32/ESP8266 microcontrollers connected to physical hardware (locks, lights, plugs). Each device subscribes only to its own command topic and reports status back to the gateway.

```
Mobile app  ──(TLS)──▶  Home gateway  ──(MQTTS)──▶  Smart devices
 face recog              Raspberry Pi                 ESP32 / ESP8266
 on-device               auth + broker                locks, lights…
```
...
- **On-device recognition** — embeddings never leave the phone
...
| Face model | ArcFace / FaceNet (quantized INT8) |
